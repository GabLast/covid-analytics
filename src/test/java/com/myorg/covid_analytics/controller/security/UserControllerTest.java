...
public class UserControllerTest extends AbstractControllerTest {
    ...
    void fetch() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/v1/user/fetch").header("Authorization", "Bearer " + token))
                .andExpect(status().is(
    ...
    void postTest() throws Exception {
        ...
        String json2 = new ObjectMapper().writeValueAsString(
                UserRequest.builder().name("Test user").build());

        mockMvc.perform(
                post("/api/v1/user").contentType(APPLICATION_JSON_VALUE).content(json2)
                        .header("Authorization", "Bearer " + token)).andExpect(
                status().is(oneOf(HttpStatus.OK.value(), HttpStatus.UNAUTHORIZED.value(),
    ...
    void getTest() throws Exception {
        ...
        Assertions.assertNotNull(token);
        mockMvc.perform(get("/api/v1/user" + "?id=" + 1).header("Authorization",
                "Bearer " + token)).andExpect(
            status().is(oneOf(HttpStatus.OK.value(), HttpStatus.NOT_FOUND.value(), HttpStatus.UNAUTHORIZED.value())));
    }
    ...
    void findall() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/v1/user/findall").header("Authorization", "Bearer " + token))
                .andExpect(status().is(
                        oneOf(HttpStatus.OK.value(), HttpStatus.UNAUTHORIZED.value())));
    }
    ...
    void countall() throws Exception {
        ...
        mockMvc.perform(
                        get("/api/v1/user/countall").header("Authorization", "Bearer " + token))
                .andExpect(status().is(
                        oneOf(HttpStatus.OK.value(), HttpStatus.UNAUTHORIZED.value())));
    }
    ...
    void deleteTest() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/user" + "?id=" + 1).header("Authorization",
